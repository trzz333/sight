---
name: sight-handoff
description: End-of-session handoff for the Sight project. Trigger when the user says "/sight-handoff", "/handoff", "handoff", "wrap up", "push and handoff", "session done", or similar, or proactively when context is heavy (30+ tool calls, long conversation, a significant phase just completed). This skill updates docs/sight-handoff.md in the canonical terse schema (phase, last commit, current task, next action, blockers, <=5 notes), commits and pushes to trzz333/sight on GitHub so GPT can see it, and ALWAYS outputs two separate bootstrap messages (one for Claude, one for GPT) each in its own copy/paste markdown code block. Both messages every time. Never combine them. Never skip GPT because the current session was Claude-facing. Trigger proactively when context is heavy, do not wait to be asked.
---
