...
class TrainMetricsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.num_timesteps >= self._next_ckpt:
            ...
            self._model.save(str(self.ckpt_dir / f"qrdqn_{tag}.zip"))
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="k5_7_qrdqn_train")
    ...
    model = QRDQN(
        ...
        max_grad_norm=10.0,
        policy_kwargs=dict(n_quantiles=int(args.n_quantiles),
    ...
    model.save(str(out_dir / "qrdqn_sb3.zip"))
    ...
    print(f"DONE wall={wall:.1f}s fps={fps:.1f} model={out_dir / 'qrdqn_sb3.zip'}",
          flush=True)
    return 0
