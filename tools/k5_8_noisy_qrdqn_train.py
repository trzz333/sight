"""K5.8 -> NoisyNet QR-DQN on Signal Dodge (state mode). From-scratch RL.

Escalation from K5.7. K5.7 (QR-DQN + n-step, epsilon-greedy) collapsed to a
single-direction basin (left-only at 50k, frac_R 0.000), the MIRROR of vanilla
DQN's right-only collapse and PPO's constant-action collapse. The env is
provably left/right SYMMETRIC, so the basin is an EXPLORATION pathology, not an
env or value-estimation problem. Raising epsilon would be "retry epsilon-greedy
harder," which the contract forbids. K5.8 REPLACES the exploration mechanism:

  NoisyNet (Fortunato et al. 2017) -- learnable per-weight Gaussian noise in the
  quantile head, resampled every train-mode forward, zeroed in eval. Epsilon-
  greedy is DISABLED (all eps = 0) so exploration is purely state-dependent
  weight noise. This is temporally-coherent "deep" exploration (cf. Osband
  et al. 2016) rather than epsilon-greedy dithering.

Everything else is held identical to K5.7 for a controlled comparison: same
GodotSignalDodgeEnv (state, headless, reward_shaping "none", max_steps 1800),
same VecNormalize(norm_obs=True, norm_reward=False), same n-step, same
n_quantiles, same net_arch [128,128], same lr/buffer/batch. The ONLY change is
the exploration mechanism. Reward "none" keeps return == episode length, whose
best constant (845.7) is below the 930.27 bar, so any clearing policy must dodge.

Output (under <out>/):
  qrdqn_noisy_sb3.zip   full SB3 model (resume + eval)
  vecnormalize.pkl      frozen obs stats for eval parity
  train_metrics.ndjson  per-rollout ep_rew_mean / ep_len_mean / loss
  train_meta.json       hyperparams + wall/fps
  ckpts/                periodic qrdqn_noisy_<steps>.zip + vecnorm_<steps>.pkl

Eval with: tools\\k5_8_noisy_eval_inenv.py (sibling; greedy, seeds 1000-1009).

Usage (cmd, SIGHT_GODOT_EXE inline):
  python tools\\k5_8_noisy_qrdqn_train.py --timesteps 200000 --seed 0
"""
...
class TrainMetricsCallback(BaseCallback):
    ...
    def __init__(self, out_path: Path, ckpt_dir: Path, ckpt_every: int,
                 model: QRDQN, vecnorm: VecNormalize):
        super().__init__()
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="k5_8_noisy_qrdqn_train")
    ...
    # NoisyNet replaces epsilon-greedy: disable epsilon entirely so the ONLY
    # exploration is weight noise. exploration_fraction=0 + all eps=0 means SB3's
    # epsilon schedule is a constant 0 and predict() during rollout is pure greedy
    # over the noisy net (noise resampled every forward because the policy is in
    # train mode during collection).
    model = QRDQN(
        ...
        policy_kwargs=dict(sigma_0=float(args.sigma0),
                           ...
                           net_arch=[128, 128], activation_fn=nn.ReLU),
    )
    ...
    print(f"DONE wall={wall:.1f}s fps={fps:.1f} model={out_dir / 'qrdqn_noisy_sb3.zip'}",
          flush=True)
    return 0
