"""K5.7 in-env greedy eval for the from-scratch DQN policy. THE verdict.

Loads dqn_sb3.zip + vecnormalize.pkl, runs greedy argmax (deterministic)
...
Usage (cmd, SIGHT_GODOT_EXE inline):
  python tools\\k5_7_dqn_eval_inenv.py --run runs\\phase_k\\k5_7_dqn
"""
...
def run(run_dir: Path, out_dir: Path, seeds: list[int], max_steps: int = 1800) -> dict:
    model = DQN.load(str(run_dir / "dqn_sb3.zip"), device="cpu")
    ...
    report = {
        "run_dir": str(run_dir), "mode": "from_scratch_dqn",
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="k5_7_dqn_eval_inenv")
