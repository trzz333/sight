"""K6 in-env greedy eval for DynQRDQN (NoisyNet QR-DQN + next-state head).

Identical eval loop and kwargs to tools\\k5_8_noisy_eval_inenv.py (same env,
seeds 1000-1009, bar 930.27, degeneracy guard) so the K6 mean is directly
comparable to K5.8 and the bar. The ONLY differences: it loads a DynQRDQN
checkpoint (and imports DynEncoder so the pickled features_extractor_class
resolves). The auxiliary dynamics head is unused at eval; action selection is
the greedy quantile head with NoisyLinear noise zeroed by eval mode.

Writes eval_inenv\\noisy_eval_inenv_report.json (same schema/name as K5.8) so
tools\\k5_8_reliability_report.py --glob can aggregate K6 runs unchanged.

Usage: python tools\\k6_dyn_eval_inenv.py --run runs\\phase_k\\k6_dyn_on_s0
"""
...
# Import custom policy + encoder + algo so SB3 unpickle / policy registry resolve.
from sight_agent.rl.noisy_qrdqn import NoisyQRDQNPolicy  # noqa: E402,F401
from sight_agent.rl.dyn_encoder import DynEncoder  # noqa: E402,F401
...
def run(run_dir: Path, out_dir: Path, seeds: list[int], max_steps: int = 1800) -> dict:
    model = DynQRDQN.load(str(run_dir / "qrdqn_noisy_sb3.zip"), device="cpu",
                          custom_objects={"policy_class": NoisyQRDQNPolicy})
    # Force eval mode so NoisyLinear noise is zeroed before the first predict().
    ...
    report = {
        "run_dir": str(run_dir), "mode": "from_scratch_dyn_qrdqn",
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="k6_dyn_eval_inenv")
